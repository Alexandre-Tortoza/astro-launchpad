...
function assertChoice<T extends readonly string[]>(value: string, choices: T, flag: string): asserts value is T[number] {
  if (!choices.includes(value)) {
    throw new Error(`Invalid value for --${flag}: ${value}. Expected one of: ${choices.join(', ')}.`);
  }
}
...
export function parseCliArguments(arguments_: string[]): CliFlags {
  ...
  for (let index = 0; index < arguments_.length; index += 1) {
    ...
    if (!argument.startsWith('--')) {
      if (flags.directory) throw new Error('Only one destination directory can be specified.');
...
export function defaultsFromFlags(flags: CliFlags, currentDirectory: string): ProjectOptions {
  const projectName = toPackageName(flags.name ?? defaultProjectName(flags.directory));
  return {
    projectName,
