...
export async function run(arguments_: string[], currentDirectory = process.cwd(), moduleUrl = import.meta.url): Promise<void> {
  ...
  const templateDirectory = join(dirname(fileURLToPath(moduleUrl)), 'template', 'base');
  ...
  if (options.install) {
    ...
    const exitCode = await runCommand(install.command, install.arguments, options.destination);
    if (exitCode !== 0) throw new Error(`Dependency installation failed with exit code ${exitCode}.`);
  }

  if (options.initializeGit) {
    const exitCode = await runCommand('git', ['init'], options.destination).catch(() => 1);
    if (exitCode !== 0) p.log.warn('Git initialization was skipped because Git is unavailable or failed.');
  }

  p.outro(`Project created in ${options.destination}\n\nNext steps:\n  cd ${options.destination}\n  ${developmentCommand(options.packageManager)}`);
}
